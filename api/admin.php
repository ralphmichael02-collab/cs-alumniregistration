...
// Handle login (now from database)
...
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    ...
    <head>
        ...
        <style>
            body { font-family: 'Outfit', sans-serif; background: #f3f1ef; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; }
            ...
            input { width: 100%; padding: 12px; margin-bottom: 1rem; border: 1px solid #cbd5e1; border-radius: 12px; }
...
// --- Admin is logged in. Process filters and pagination ---
...
// Get distinct courses and years for filter dropdowns
...
// Total registered users (unfiltered)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-group select, .filter-group input { width: 100%; padding: 10px 12px; border: 1.5px solid #e2e8f0; border-radius: 12px; font-family: inherit; }
...
<div class="dashboard-container">
    <div class="header">
        ...
        <div>
            <span style="margin-right: 1rem;">Welcome, <?= htmlspecialchars($_SESSION['admin_username']) ?> (<?= $_SESSION['admin_role'] ?>)</span>
            <a href="?logout=1" class="logout-btn no-print">Logout</a>
        </div>
    </div>

    <div class="stats-card">
        <div class="stats-number"><?= $total_users ?></div>
        <div>Total Registered Alumni</div>
    </div>
    ...
    <div class="table-wrapper">
        <table>
            <thead>
                <tr>
                    <th>ID</th><th>Last Name</th><th>First Name</th><th>Middle Name</th><th>Birthdate</th><th>Age</th><th>Gender</th><th>Civil Status</th><th>Address</th><th>Course</th><th>Year Graduated</th><th>Employment Status</th><th>Occupation</th><th>Company</th><th>Work Address</th><th>Years of Service</th><th>Recent Work</th><th>Registered On</th>
                </tr>
            </thead>
            <tbody>
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    foreach ($csv_data as $row) {
        fputcsv($output, [
            $row['id'], $row['last_name'], $row['first_name'], $row['middle_name'],
            $row['birth_date'], $row['age'], $row['gender'], $row['civil_status'],
            $row['address'], $row['course'], $row['year_graduated'],
            $row['employment_status'], $row['occupation'], $row['company_agency'],
            $row['work_address'], $row['years_of_service'], $row['recent_work'],
            $row['created_at']
        ]);
    }
    fclose($output);
